...
class TestScenarioManager:
    ...
    def test_scenario_manager_success_mode(self, mock_settings: MockSettings) -> None:
        ...
        assert manager.should_timeout(TaskName.START_CC) is False
        assert manager.should_fail(TaskName.START_CC) is False
    ...
    def test_scenario_manager_timeout_injection(self) -> None:
        ...
        assert manager.should_timeout(TaskName.TAKE_PHOTO) is True
    ...
    def test_scenario_manager_failure_injection(self) -> None:
        ...
        assert manager.should_fail(TaskName.SETUP_CARTRIDGES) is True
    ...
    def test_scenario_manager_default_failure(self) -> None:
        ...
        assert manager.should_fail(TaskName.START_EVAPORATION) is True
...
class TestFailureMessages:
    ...
    def test_failure_messages_all_tasks(self) -> None:
        """Every TaskName has entries in FAILURE_MESSAGES."""
        for task_name in TaskName:
            assert task_name in FAILURE_MESSAGES, f"Missing FAILURE_MESSAGES entry for {task_name}"
            assert len(FAILURE_MESSAGES[task_name]) > 0, f"Empty messages for {task_name}"

    def test_get_random_failure_returns_valid(self) -> None:
        """For each TaskName, get_random_failure returns (int, str)."""
        for task_name in TaskName:
            code, msg = get_random_failure(task_name)
