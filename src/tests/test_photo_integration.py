...
class TestPhotoIntegrationWorkflow:
    ...
    async def test_cc_workflow_photo_after_start(self, photo_sim, world_state):
        ...
        result = await photo_sim.simulate("photo_001", TaskName.TAKE_PHOTO, params)
        ...
        assert result.updates[0].properties.state == "watch_column_machine_screen"
    ...
    async def test_evaporation_workflow_photo_during_evaporation(self, photo_sim, world_state):
        ...
        result = await photo_sim.simulate("photo_002", TaskName.TAKE_PHOTO, params)
        ...
        # Robot update
        assert result.updates[0].type == "robot"
        assert result.updates[0].properties.state == "observe_evaporation"
        ...
        assert evap_update.properties.running is True
    ...
    async def test_cc_workflow_photo_after_terminate(self, photo_sim, world_state):
        ...
        result = await photo_sim.simulate("photo_003", TaskName.TAKE_PHOTO, params)

        # 3. Verify response includes terminated state
        assert result.is_success()
        assert len(result.updates) == 2
    ...
    async def test_multiple_photos_preserve_device_state(self, photo_sim, world_state):
        ...
        result1 = await photo_sim.simulate("photo_004", TaskName.TAKE_PHOTO, params1)
        ...
        result2 = await photo_sim.simulate("photo_005", TaskName.TAKE_PHOTO, params2)
        ...
        assert evap1.properties.running == evap2.properties.running
