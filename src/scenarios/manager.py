...
class ScenarioManager:
    ...
    def should_timeout(self, task_name: TaskName | str) -> bool:
        ...
        if self._timeout_rate > 0 and random.random() < self._timeout_rate:  # noqa: S311
            logger.info("Scenario: TIMEOUT injected for task {}", task_name)
            return True
        return False

    def should_fail(self, task_name: TaskName | str) -> bool:
        ...
        if self._failure_rate > 0 and random.random() < self._failure_rate:  # noqa: S311
            logger.info("Scenario: FAILURE injected for task {}", task_name)
            return True
        # Check default scenario
        if self._default_scenario == "failure":
            logger.info("Scenario: FAILURE (default) for task {}", task_name)
            return True
        return False

    def get_failure_result(self, task_id: str, task_name: TaskName | str) -> RobotResult:
        """Generate a failure RobotResult with task-specific error."""
        code, msg = get_random_failure(TaskName(task_name))
        logger.warning(
